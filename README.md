...
platform/
├── packages/
│   ├── sdk/            @freebiostore/sdk — model loading, inference, Ollama, heuristics
│   ├── cli/            @freebiostore/cli — `fags` binary (init, check, publish)
│   ├── compliance/     Agent-specific compliance checks (9 checks)
│   ├── backend/        API Worker (placeholder)
│   └── quality/        Quality reporter (placeholder)
├── workers/
│   ├── host/           *.freebiostore.online → D1 → R2 host worker
│   └── mcp/            MCP server (11 tools) at mcp.freebiostore.online
├── agents/
...
import { useModel, useWorkerInference, useOllama } from '@freebiostore/sdk/hooks';
...
import { evaluateHeuristic, buildEvolvePrompt } from '@freebiostore/sdk';
```

## MCP tools

Connect via `npx mcp-remote https://mcp.freebiostore.online/mcp`
...
| Resource | Name/ID |
|---|---|
| CF Zone | `freebiostore.online` |
| D1 | `fags` |
| R2 | `fbs-items` |
| KV (OAuth) | `FAGS_OAUTH_KV` |
| Doppler | project `fags` |
| GitHub Org | `FreeBioStore` |
...
| **FreeBioStore** | **Browser AI tools** | **https://freebiostore.online** |
