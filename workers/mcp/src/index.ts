...
export class FagsMcpAgent extends McpAgent<Env, unknown, McpProps> {
  ...
  async init() {
    ...
    this.server.tool(
      'list_agents',
      'List all published agents on FreeBioStore, or just your own if authenticated.',
      ...
      async ({ mine }) => {
        ...
        const lines = rows.results.map(
          (r) => `- **${r.slug}** → https://freebiostore.online/a/${r.slug}/`,
        );
    ...
    this.server.tool(
      ...
      async ({ agent_id }) => {
        ...
        const liveUrl = `https://freebiostore.online/a/${agent_id}/`;
    ...
    this.server.tool(
      'create_agent',
      'Create a new agent on FreeBioStore — provisions GitHub repo, R2 route, and DNS. Requires auth + GITHUB_TOKEN.',
      {
        agent_id: z
          .string()
          .regex(/^[a-z0-9-]+$/)
      ...
      async ({ agent_id, name, description, template }) => {
        ...
        const existing = await this.env.DB.prepare(
          "SELECT slug FROM routes WHERE slug = ? AND zone = 'freebiostore.online'",
        )
          .bind(agent_id)
          .first();
        if (existing)
          return text(
            `Agent **${agent_id}** already exists at https://freebiostore.online/a/${agent_id}/`,
          );
        ...
        await this.env.DB.prepare(
          "INSERT INTO routes (slug, zone, r2_prefix, store, hosted_on, created_at, updated_at) VALUES (?, 'freebiostore.online', ?, 'agents', 'r2', strftime('%s','now'), strftime('%s','now'))",
        )
        ...
        return text(
          [
            ...
            `URL: https://freebiostore.online/a/${agent_id}/ (live after first deploy)`,
            ``,
            `Next steps:`,
    ...
    this.server.tool(
      ...
      async ({ agent_id, archive_repo }) => {
        ...
        const result = await this.env.DB.prepare(
          "DELETE FROM routes WHERE slug = ? AND zone = 'freebiostore.online'",
        )
          .bind(agent_id)
          .run();
    ...
    this.server.tool(
      'platform_guide',
      'Get the FreeBioStore platform guide — architecture, SDK reference, and how to build browser-based AI agents.',
      {},
      async () => {
        // Return a condensed guide
        return text(
          [
            '# FreeBioStore Platform Guide',
            '',
            '## What it is',
            ...
            '',
            '## SDK: @freebiostore/sdk',
            '```tsx',
            "import { initAgent } from '@freebiostore/sdk'",
            "const agent = initAgent({ agentId: 'my-agent' })",
            '',
            '// Hooks for React:',
            "import { useModel, useWorkerInference, useOllama, useResultStore } from '@freebiostore/sdk/hooks'",
            '```',
            '',
            '## Templates',
            ...
            '',
            '## Deploy',
            'Push to main → GitHub Actions → R2 → live at freebiostore.online/a/{agent}/',
            '',
            '## Compliance',
    ...
    this.server.tool(
      'sdk_reference',
      'Quick reference for @freebiostore/sdk — model loading, inference, Ollama, storage.',
      {
        feature: z
      ...
      async ({ feature }) => {
        const sections: Record<string, string> = {
          model: `## Model Loading
\`\`\`tsx
import { useModel } from '@freebiostore/sdk/hooks'
...
\`\`\`tsx
import { useWorkerInference } from '@freebiostore/sdk/hooks'
...
\`\`\`tsx
import { useOllama } from '@freebiostore/sdk/hooks'
...
Ollama at localhost:11434. User sets OLLAMA_ORIGINS="*" for CORS.`,
          cache: `## Model Cache
\`\`\`tsx
import { useModelCache } from '@freebiostore/sdk/hooks'
...
\`\`\`tsx
import { useResultStore } from '@freebiostore/sdk/hooks'
...
\`\`\``,
        ...
        return text(`# @freebiostore/sdk Reference\n\n${selected}`);
      },
    );
  }
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (url.pathname === '/' || url.pathname === '') {
      return new Response(
        [
          'FreeBioStore MCP Server',
          '',
          'Connect: npx mcp-remote https://mcp.freebiostore.online/mcp',
          '',
          'Tools:',
